...
- **Tests RPC end-to-end avec auth.uid simulé** : compléter `tests/period-closure.test.ts` avec un 6e cas qui valide le flow complet de close_period() (insert closure + flag rows + update companies) en simulant `auth.uid()` dans la connection pg directe (set role + jwt.claims.sub). Aujourd'hui testé manuellement via UI. V1.5.
